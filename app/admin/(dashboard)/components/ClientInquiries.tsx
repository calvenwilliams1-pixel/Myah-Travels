        ...
        <ul className="space-y-3">
          {clients.map((client: any) => (
            <li key={client.id} className="flex items-center justify-between gap-3">
              <Link href={`/admin/clients/${client.id}`} className="text-sm text-gray-700 hover:text-primary truncate flex-1 min-w-0">
                {client.fullName}
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${
                ...
                client.status === "booked" ? "bg-primary/10 text-primary" :
