...
// Ensure color_presets exists (Phase 7.9)
try {
  db.exec(`
  ...
  `);
  console.log("  color_presets ready");
} catch (err) {
  console.error("  color_presets:", err.message);
}

db.close();

// Seed colour presets on fresh install (Phase 7.9)
try {
  const { execSync } = require("child_process");
  execSync("npx tsx scripts/seed-color-presets.ts", { stdio: "inherit" });
} catch (err) {
