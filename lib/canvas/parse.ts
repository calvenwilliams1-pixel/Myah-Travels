...
export const MAX_CANVAS_JSON_LENGTH = 500_000;
...
export function parseCanvasDocument(
  ...
  if (json.length > MAX_CANVAS_JSON_LENGTH) {
    if (debug) console.warn("[parseCanvasDocument] JSON too large:", json.length);
    return null;
  }
  ...
  if (typeof doc !== "object" || doc === null) {
    if (debug) console.warn("[parseCanvasDocument] Not an object");
    return null;
  }
  ...
  if (!isCanvasDocument(doc)) {
    if (debug) console.warn("[parseCanvasDocument] Validation failed");
    return null;
  }

  const cloned = JSON.parse(JSON.stringify(doc)) as CanvasDocument;
